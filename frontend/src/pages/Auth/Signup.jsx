...
export default function Signup() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (d?.error === 'validation' && d.messages) setErr(Object.values(d.messages).join('; '));
  ...
  return (
    <div className="max-w-md mx-auto p-6 rounded-lg shadow-lg bg-[var(--card)]/90 backdrop-blur-sm border border-[var(--glass)]">
      <h2 className="text-2xl font-semibold mb-6 text-center text-[var(--accent)]">Create Account</h2>
      <form onSubmit={submit} className="space-y-4">
        <label className="block">
          <div className="text-sm mb-1 text-[var(--muted)]">Name</div>
          <input
            value={name}
